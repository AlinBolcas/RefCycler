...
class ImageCyclerApp(ctk.CTk):
    def __init__(self):
        ...
        self.conversation_buffer = ConversationBuffer()  # Initialize conversation buffer
        ...
        self.stop_button = ctk.CTkButton(self, text="Cycle", command=self.toggle_cycling)
        self.stop_button.grid(row=1, column=3, padx=20, pady=20, sticky="ew")

        self.screenshot_button = ctk.CTkButton(self, text="Feedback", command=self.take_screenshot)
        self.screenshot_button.grid(row=1, column=4, padx=20, pady=20, sticky="ew")
        ...
        # Add AI response text field
        self.ai_response_label = ctk.CTkLabel(self, text="AI Response:")
        self.ai_response_label.grid(row=3, column=0, padx=20, pady=10, sticky="w")

        self.ai_response_text = scrolledtext.ScrolledText(self, height=15, wrap=ctk.WORD, bg=self.cget("background"), fg="white", insertbackground="white", bd=0, relief="flat")
        self.ai_response_text.grid(row=4, column=0, columnspan=8, padx=20, pady=10, sticky="ew")

        # Add user input text field
        self.user_input_label = ctk.CTkLabel(self, text="Your Input:")
        self.user_input_label.grid(row=5, column=0, padx=20, pady=10, sticky="w")

        self.user_input_text = ctk.CTkTextbox(self, height=5, width=100)
        self.user_input_text.grid(row=6, column=0, columnspan=6, padx=20, pady=10, sticky="ew")

        # Add send button
        self.send_button = ctk.CTkButton(self, text="Send", command=self.send_message)
        self.send_button.grid(row=6, column=6, padx=20, pady=10, sticky="ew")

        # Make Enter key press the send button
        self.user_input_text.bind('<Return>', lambda event: self.send_button.invoke())
        ...
        keyboard.add_hotkey('ctrl+shift+a', lambda: [self.focus_app(), self.take_screenshot()])
    ...
    def send_message(self):
        user_message = self.user_input_text.get("1.0", "end").strip()
        if user_message:
            ...
            self.ai_response_text.insert("end", f"\nUser: {user_message}\n")
    ...
    def process_message(self, user_message):
        try:
            ...
            ] + [{"role": role.lower(), "content": message} for role, message, _ in self.conversation_buffer.buffer]
            ...
            if response and response.choices and len(response.choices) > 0:
                ...
                self.conversation_buffer.add_message("assistant", ai_message)
                self.after(0, self.display_ai_message, ai_message)
            else:
                self.after(0, self.display_ai_message, "No response generated.")
        except Exception as e:
            print(f"Error processing message: {e}")
            self.after(0, self.display_ai_message, f"Error: {e}")

    def take_screenshot(self):
        print("Starting screenshot process...")
        threading.Thread(target=self.start_screenshot_capture).start()
    ...
    def capture_and_critique_screenshot(self, screenshot_path):
        def task():
            ...
            self.after(0, self.process_critique_result, critique)

        print("Starting screenshot capture and critique process...")
        threading.Thread(target=task).start()

    def process_critique_result(self, critique):
        try:
            print("Artistic Critique received:", critique)
            if critique:
                print("Adding critique to conversation buffer...")
                self.conversation_buffer.add_message("assistant", critique)
                print("Updating UI with critique...")
                self.display_ai_message(critique)  # Update UI in main thread
            print("Finished processing screenshot critique.")
        except Exception as e:
            print(f"Error in processing critique result: {e}")
            self.display_ai_message(f"Error: {e}")  # Update UI in main thread



    def display_ai_message(self, message):
        print("Displaying AI message on UI.")
        self.ai_response_text.insert("end", f"\n------\n\nAI: {message}\n")
        self.ai_response_text.see("end")
        print("AI message displayed on UI.")
    ...
    def toggle_cycling(self):
        if self.stop_event.is_set():
            ...
            self.stop_event.set()
            self.stop_button.configure(text="Cycle")
